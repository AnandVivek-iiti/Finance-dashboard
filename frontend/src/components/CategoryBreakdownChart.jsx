...
export default function CategoryBreakdownChart({ data, showValues = false }) {
  ...
  return (
    <ChartCard title="Spending by Category" subtitle="Top 10 categories by total amount" chartKey="spendByCategory">
      {rows.length === 0 ? (
        <EmptyNote />
      ) : (
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={rows} layout="vertical" margin={{ left: 10, right: showValues ? 60 : 10 }} accessibilityLayer={false}>
            ...
            />
            <Bar dataKey="amount" radius={[0, 4, 4, 0]} maxBarSize={18} isAnimationActive={!showValues}>
              {rows.map((_, i) => (
              ...
              {showValues && (
                <LabelList
                  ...
                  style={{ fontSize: 10, fill: "#171b2b", fontWeight: 600 }}
                />
              )}
            </Bar>
          </BarChart>
